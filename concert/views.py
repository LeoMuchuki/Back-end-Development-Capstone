...
def songs(request):
    songs = req.get("http://songs-sn-labs-lennymkuu.labs-prod-openshift-san-a45631dc5778dc6371c67d206ba9ae5c-0000.us-east.containers.appdomain.cloud/song").json()
    return render(request, "songs.html", {"songs":songs["songs"]})


def photos(request):
    photos = req.get("https://pictures.1i0yb0v76lvv.us-south.codeengine.appdomain.cloud/picture").json()
...
def concerts(request):
    if request.user.is_authenticated:
        concerts_list = list()
        concerts_objects = Concert.objects.all()
        for item in concerts_objects:
            try:
